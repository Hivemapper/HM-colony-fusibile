...
int getCameraFromId(string id, vector<Camera> &cameras) {
  for (size_t i = 0; i < cameras.size(); i++) {
    // cout << "Checking camera id " << i << " cameraid " << cameras[i].id <<
    // endl;
 ...
 * Output: inputFiles, outputFiles, parameters, gt_parameters, no_display -
...
static int getParametersFromCommandLine(int argc, char **argv,
  ...
  const char *border_value =
      "--border_value="; // either constant scalar or -1 = REPLICATE
  ...
  for (int i = 1; i < argc; i++) {
    if (argv[i][0] != '-') {
      ...
      if (parameters.cost_comb < 0) {
        printf("Command-line parameter error: Unknown cost combination "
               "method\n\n");
        print_help();
        return -1;
      }
    ...
    else if (strncmp(argv[i], normal_thresh_opt, strlen(normal_thresh_opt)) ==
             0) {
      float angle_degree;
      sscanf(argv[i] + strlen(normal_thresh_opt), "%f", &angle_degree);
      parameters.normalThresh = angle_degree * M_PI / 180.0f;
    } else if (strncmp(argv[i], num_consistent_opt,
                       strlen(num_consistent_opt)) == 0)
      sscanf(argv[i] + strlen(num_consistent_opt), "%d",
             &parameters.numConsistentThresh);
    else {
...
static void selectViews(CameraParameters &cameraParams, int imgWidth,
  ...
  for (size_t i = 0; i < cameras.size(); i++) {
    ...
    if (angle > minimum_angle_radians &&
        angle < maximum_angle_radians) // 0.6 select if angle between 5.7
                                       // and 34.8 (0.6) degrees (10 and 30
                                       // degrees suggested by some paper)
    {
      cameraParams.viewSelectionSubset.push_back(i);
...
static void addImageToTextureUint(vector<Mat_<uint8_t>> &imgs,
  ...
  for (unsigned int i = 0; i < imgs.size(); i++) {
    ...
    checkCudaErrors(cudaMemcpy2DToArray(cuArray, 0, 0, imgs[i].ptr<uint8_t>(),
                                        imgs[i].step[0], cols * sizeof(uint8_t),
...
static void addImageToTextureFloatColor(vector<Mat> &imgs,
  ...
  for (size_t i = 0; i < imgs.size(); i++) {
    ...
    checkCudaErrors(cudaMemcpy2DToArray(
        cuArray, 0, 0, imgs[i].ptr<float>(), imgs[i].step[0],
...
static void addImageToTextureFloatGray(vector<Mat> &imgs,
  ...
  for (size_t i = 0; i < imgs.size(); i++) {
    ...
    checkCudaErrors(cudaMemcpy2DToArray(cuArray, 0, 0, imgs[i].ptr<float>(),
                                        imgs[i].step[0], cols * sizeof(float),
                                        rows, cudaMemcpyHostToDevice));
...
static int runFusibile(int argc, char **argv,
  ...
  algParameters.num_img_processed =
      min((int)numImages, algParameters.num_img_processed);
  ...
  for (size_t i = 0; i < numImages; i++) {
    ...
    // inputFiles.img_filenames[i] ).c_str () );
    img_grayscale.push_back(
  ...
  CameraParameters camParams = getCameraParameters(
      *(gs->cameras), inputFiles, algParameters.depthMin,
      algParameters.depthMax, algParameters.cam_scale, false);
  printf("Camera size is %lu\n", camParams.cameras.size());

  for (int i = 0; i < algParameters.num_img_processed; i++) {
    algParameters.min_disparity =
        disparityDepthConversion(camParams.f, camParams.cameras[i].baseline,
  ...
  for (map<int, string>::iterator it = consideredIds.begin();
       it != consideredIds.end(); ++it) {
  ...
  for (size_t i = 0; i < img_grayscale.size(); i++) {
    // img_grayscale[i].convertTo(img_grayscale_float[i], CV_32FC1, 1.0/255.0);
    // // or CV_32F works (too)
    ...
    if (algParameters.color_processing) {
      ...
      img_color_float_alpha[i] =
          Mat::zeros(img_grayscale[0].rows, img_grayscale[0].cols, CV_32FC4);
...
int main(int argc, char **argv) {
  ...
  int ret =
      getParametersFromCommandLine(argc, argv, inputFiles, outputFiles,
                                   *algParameters, gtParameters, no_display);
  if (ret != 0)
    return ret;
